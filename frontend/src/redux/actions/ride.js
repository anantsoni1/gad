...
export const getUpcomingRides = ( ) => async (dispatch) => {
    try {
        const { data } = await api.getUpcomingRides();
        dispatch({ type: GET_UPCOMING_RIDES, data });
    } catch (e) {
        console.log(e);
        swal({
            text: e.response.data.msg,
...
export const confirmRide = ( formData , history) => async (dispatch) => {
    try {
        if (!localStorage.getItem("token")) {
            swal({
                text: "You need to login before you can book rides",
                icon: "info",
            });
            history.push("/login");
            return;
...
export const confirmBooking = ( token , history) => async (dispatch) => {
    try {
        ...
        console.log(data);
        history.push("/client/ride-details?id="+data?.ride?._id);
    } catch (e) {
        console.log(e);
        swal({
            text: e.response.data.msg,

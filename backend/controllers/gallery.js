...
exports.addGallerydata = async (req, res) => {
  ...
  req.body.images.map((val) => {
    ...
    let fileName = val.name.replace(/\s/g, "") + "." + extension;
...
exports.modifyGalleryData = async (req, res) => {
  ...
  gallery.findByIdAndUpdate(req.query.id, newDataObj, (err, data) => {
    if (err) {
